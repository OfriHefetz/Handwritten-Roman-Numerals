...
# clean data
# base_dir = os.path.join("..", "my_data")
# train_dir = os.path.join(base_dir, "train")
# val_dir = os.path.join(base_dir, "val")
#augmantation
base_dir = os.path.join("..", "augmentation_data")
train_dir = os.path.join(base_dir, "train")
val_dir = os.path.join(base_dir, "val")
...
dataloaders = {'all_data': train_dataloader, 'val': val_dataloader}
dataset_sizes = {'all_data': len(train_dataset), 'val': len(val_dataset)}
...
def train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, num_epochs=100):
    ...
    best_acc = 0.0
    loss_dict = {'all_data': [], 'val': []}
    acc_dict = {'all_data': [], 'val': []}

    for epoch in range(num_epochs):
        print('Epoch {}/{}'.format(epoch + 1, num_epochs))
        print('-' * 10)

        for phase in ['all_data', 'val']:
            if phase == 'all_data':
                model.train() 
            else:
                model.eval()   
            ...
            for inputs, labels in tqdm(dataloaders[phase]):
                ...
                # track history only in all_data
                with torch.set_grad_enabled(phase == 'all_data'):
...
plt.plot(loss_dict['all_data'], label='all_data')
...
plt.plot(acc_dict['all_data'], label='all_data')
